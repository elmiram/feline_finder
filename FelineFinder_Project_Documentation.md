...
- **`feline-finder-frontend/`**: The root directory for the React project.
  ...
  - **`src/`**: Contains all the application's source code.
    ...
    - **`App.js`**: The main application component. It acts as a container, managing top-level state (like the active view and fetched data) and rendering the correct view (`DashboardView` or `HistoryView`).
    - **`constants.js`**: Centralizes application-wide constants, such as the `API_BASE_URL`, making configuration easy to manage.
